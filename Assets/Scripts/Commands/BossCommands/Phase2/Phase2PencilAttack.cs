...
public class Phase2PencilHarderAttack : BossCommand
{
    public Phase2PencilHarderAttack(BossController boss) : base(boss) { }
    bool isOdd = true;
    ...
    public override IEnumerator Execute()
    {
        ...
        for (int k = 0; k < attackCount; k++)
        {
            ...
            randomIndexes = GetRandomUniqueEvenNumbers(Random.Range(1, 2), 0, size);
            vertical = !vertical;
            for (int i = 0; i < size; i += 2)
            {
                if (!randomIndexes.Contains(i))
